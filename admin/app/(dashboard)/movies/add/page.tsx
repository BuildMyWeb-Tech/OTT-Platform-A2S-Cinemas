...
import { ArrowLeft, X, Film, Image as ImageIcon, ChevronDown, Check } from "lucide-react";
...
interface UploadState {
    file: File | null; progress: number; url: string; uploading: boolean; error: string;
}
const emptyUpload = (): UploadState => ({ file: null, progress: 0, url: "", uploading: false, error: "" });
...
export default function AddMoviePage() {
    ...
    ) => { setForm((f) => ({ ...f, [key]: e.target.value })); setErrors((e) => ({ ...e, [key]: "" })); };
    ...
    const uploadPoster = async (file: File) => {
        ...
        try {
            ...
            if (!cloudName) { setPoster((p) => ({ ...p, uploading: false, error: "Cloudinary not configured" })); return; }
            ...
            xhr.upload.onprogress = (e) => {
                if (e.lengthComputable) setPoster((p) => ({ ...p, progress: Math.round((e.loaded / e.total) * 100) }));
            };
            const result = await new Promise<string>((resolve, reject) => {
                xhr.onload = () => xhr.status === 200 ? resolve(JSON.parse(xhr.response).secure_url) : reject(new Error("Upload failed"));
    ...
    const uploadVideo = async (file: File) => {
        ...
        try {
            const { data } = await api.post("/admin/upload-url", { fileName: file.name, fileType: file.type, folder: "videos" });
            const { uploadUrl, key } = data.data;
            await new Promise<void>((resolve, reject) => {
                const xhr = new XMLHttpRequest();
                xhr.upload.onprogress = (e) => {
                    if (e.lengthComputable) setVideo((v) => ({ ...v, progress: Math.round((e.loaded / e.total) * 100) }));
                };
            ...
            <div className="max-w-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-[#111118] border border-[#1E1E2E] rounded-xl p-5">
                        <h3 className="text-white font-medium text-sm mb-4 pb-2 border-b border-[#1E1E2E]">Poster Image (Cloudinary)</h3>
                        ...
                        {!poster.url ? (
                            <div>
                                <div onClick={() => posterInputRef.current?.click()}
                    ...
                    {/* Video */}
                    <div className="bg-[#111118] border border-[#1E1E2E] rounded-xl p-5">
                        <h3 className="text-white font-medium text-sm mb-4 pb-2 border-b border-[#1E1E2E]">Video File (AWS S3)</h3>
                        <input ref={videoInputRef} type="file" accept="video/mp4,video/mov,video/avi" className="hidden"
                                ...
                                <div onClick={() => videoInputRef.current?.click()}
                                    ...
                                    {video.uploading ? (
                                        ...
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            <p className="text-gray-600 text-xs">MP4, MOV — direct S3 upload</p>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="bg-[#111118] border border-[#1E1E2E] rounded-xl p-5 space-y-4">
                        ...
                        <Input label="Title" placeholder="Enter movie title" value={form.title}
                            onChange={setField("title")} error={errors.title} data-testid="movie-title-input" />

                        <Textarea label="Description" placeholder="Movie description..." value={form.description}
                            onChange={setField("description")} error={errors.description} rows={3} />
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Input label="Price (₹)" type="number" min="1" placeholder="49" value={form.price}
                                onChange={setField("price")} error={errors.price} data-testid="movie-price-input" />
