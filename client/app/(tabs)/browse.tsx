...
export default function Browse() {
    ...
    api.get("/movies?limit=100").then(({ data }) => {
        const movies = data.data || [];
        // Build a set of category IDs/slugs that have movies
        const catIdsWithMovies = new Set<string>();
        movies.forEach((m: any) => {
            if (m.categories) {
                m.categories.forEach((c: any) => {
                    catIdsWithMovies.add(typeof c === "string" ? c : c._id);
                    if (c.slug) catIdsWithMovies.add(c.slug);
                });
            }
        });
        ...
        api.get("/categories").then(({ data: catData }) => {
            ...
            // Only keep categories that have at least one movie
            const filtered = allCats.filter((cat: any) =>
                catIdsWithMovies.has(cat._id) || catIdsWithMovies.has(cat.slug)
            );
...
>
                    ...
                    <TextInput
                        ref={searchInputRef}
                        style={{ flex: 1, marginLeft: 10, fontSize: 15, color: colors.inputText }}
            ...
            {!isSearching && (
                <View style={{ marginTop: 8, marginBottom: 4 }}>
                    ...
                    >
                        ...
                        >
                            <Text style={{ fontSize: 13, fontWeight: "600", color: selectedCategory === "all" ? "#fff" : colors.textSecondary }}>
                                All
                            </Text>
                        </TouchableOpacity>
                        {categories.map((cat) => {
                            ...
                            return (
                                <TouchableOpacity
                                    key={cat._id} onPress={() => handleCategorySelect(cat.slug)}
                                    style={{
                                        paddingHorizontal: 18, paddingVertical: 8, borderRadius: 20,
                                ...
                                >
                                    <Ionicons name={getCategoryIcon(cat.name) as any} size={13} color={isSelected ? "#fff" : colors.textSecondary} />
            ...
            <View style={{
                ...
                >
                    ...
                    <Text style={{ fontSize: 13, color: colors.textSecondary, fontWeight: "500" }}>Filter</Text>
                </TouchableOpacity>
            </View>
        </SafeAreaView>
    ...
    removeClippedSubviews={true}
                renderItem={({ item }) => (
                    <BrowseCard
                        ...
                        owned={hasLicense(item._id)} daysLeft={getDaysLeft(item._id)}
                    />
                )}
                ...
                ListEmptyComponent={(
                    <View style={{ alignItems: "center", paddingTop: 80, paddingHorizontal: 32 }}>
                        ...
                        <Text style={{ color: colors.textMuted, fontSize: 14, textAlign: "center", lineHeight: 20 }}>
                            {isSearching ? `No results for "${search}". Try a different keyword.` : "No movies in this category yet."}
                        </Text>
                        {isSearching && (
                            <TouchableOpacity onPress={clearSearch} style={{ marginTop: 16 }}>
                                <Text style={{ color: colors.accent, fontSize: 14, fontWeight: "600" }}>Clear search</Text>
                            </TouchableOpacity>
                        )}
                    </View>
                )}
        ...
        {/* ── SUGGESTIONS DROPDOWN — rendered at root level, floats over everything ── */}
        {showSuggestions && suggestions.length > 0 && (
            <View style={{
                position: "absolute",
                // Position below the search bar — SafeAreaView top + header padding + title + searchbar
                top: 130,
                left: 16, right: 16,
                ...
                >
                    {suggestions.map((s, i) => (
                        ...
                        >
                            {s.poster ? (
                                <Image source={{ uri: s.poster }} style={{ width: 36, height: 50, borderRadius: 6 }} resizeMode="cover" />
                            ) : (
                                <View style={{ width: 36, height: 50, borderRadius: 6, backgroundColor: colors.surfaceVariant, justifyContent: "center", alignItems: "center" }}>
                                    <Ionicons name="film-outline" size={16} color={colors.textMuted} />
                                </View>
                            )}
                            <View style={{ flex: 1 }}>
                                <Text style={{ fontSize: 14, fontWeight: "600", color: colors.textPrimary }} numberOfLines={1}>{s.title}</Text>
                                <Text style={{ fontSize: 12, color: colors.accent, marginTop: 2, fontWeight: "600" }}>₹{s.price}</Text>
                            </View>
        ...
        {/* Category filter modal — unchanged */}
        <Modal visible={showCategoryModal} transparent animationType="slide" onRequestClose={() => setShowCategoryModal(false)}>
            <TouchableOpacity style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)" }} activeOpacity={1} onPress={() => setShowCategoryModal(false)} />
            <View style={{ backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingBottom: 40, maxHeight: "65%" }}>
                ...
                <View style={{
                    ...
                    <Text style={{ fontSize: 17, fontWeight: "700", color: colors.textPrimary }}>Filter by Category</Text>
                    <TouchableOpacity
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    <CategoryFilterRow label="All Categories" icon="grid-outline" isSelected={selectedCategory === "all"} onPress={() => handleCategorySelect("all")} colors={colors} />
                    {categories.map((cat) => (
                        <CategoryFilterRow key={cat._id} label={cat.name} icon={getCategoryIcon(cat.name)} isSelected={selectedCategory === cat.slug} onPress={() => handleCategorySelect(cat.slug)} colors={colors} />
                    ))}
                </ScrollView>
            </View>
        </Modal>
...
function BrowseCard({ movie, colors, onPress, owned, daysLeft }: {
    ...
    return (
        <TouchableOpacity onPress={onPress} activeOpacity={0.82} style={{ width: CARD_W, marginBottom: 16 }}>
            <View style={{
                ...
                {!owned && (
                    <View style={{
                        ...
                        <Text style={{ color: "#fff", fontSize: 11, fontWeight: "700" }}>₹{movie.price}</Text>
                    </View>
                )}
                ...
                {owned && (
                    <View style={{
                        ...
                        <Text style={{ color: "#fff", fontSize: 10, fontWeight: "700" }}>{daysLeft}d left</Text>
                    </View>
                )}
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 4, marginTop: 3 }}>
                        ...
                        {movie.duration ? (
                            <>
                                ...
                                <Text style={{ color: "rgba(255,255,255,0.65)", fontSize: 10 }}>{movie.duration}m</Text>
                            </>
                        ) : null}
                    </View>
                </View>
