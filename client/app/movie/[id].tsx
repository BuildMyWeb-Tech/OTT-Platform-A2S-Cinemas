...
function StarRating({ rating, onRate, size = 20, readonly = false }: {
    ...
    return (
        <View style={{ flexDirection: "row", gap: 4 }}>
            {[1, 2, 3, 4, 5].map((star) => (
                <TouchableOpacity key={star} onPress={() => !readonly && onRate?.(star)} disabled={readonly} activeOpacity={readonly ? 1 : 0.7}>
                    <Ionicons name={star <= rating ? "star" : "star-outline"} size={size} color={star <= rating ? "#FFD700" : "#666"} />
                </TouchableOpacity>
            ))}
        </View>
    );
...
export default function MovieDetail() {
    ...
    const fetchMovie = async () => {
        try {
            ...
            if (error?.response?.status === 404) setMovieDeleted(true);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            Toast.show({ type: "error", text1: "Failed", text2: error.response?.data?.message || "Could not submit review" });
        } finally {
    ...
    const handleBuy = async () => {
        ...
        try {
            ...
            if (!orderRes.data.success) {
                Toast.show({ type: "error", text1: "Order failed", text2: orderRes.data.message || "Failed to create order" });
                return;
            }
            ...
            try {
                ...
                if (code === 0 || desc.toLowerCase().includes("cancel")) {
                    ...
                    Toast.show({ type: "error", text1: "Payment failed", text2: desc || "Something went wrong in checkout" });
                }
                return;
            }
            const paymentId = rzpData.razorpay_payment_id || rzpData.paymentId || rzpData.payment_id || "";
            const returnedOrderId = rzpData.razorpay_order_id || rzpData.orderId || rzpData.order_id || orderId;
            const signature = rzpData.razorpay_signature || rzpData.signature || "";
            if (!paymentId || !signature) {
                Toast.show({ type: "error", text1: "Verification failed", text2: "Incomplete payment response. Please contact support." });
                return;
            }
            ...
            try {
                ...
                Toast.show({ type: "error", text1: "Verification error", text2: verifyError?.response?.data?.message || "Verification request failed" });
                return;
            }
            if (verifyRes?.data?.success) {
                await fetchLicenses();
                await fetchMovie();
                router.replace({ pathname: "/payment/callback", params: { status: "success", movie_id: id as string } } as any);
            } else {
                Toast.show({ type: "error", text1: "Verification failed", text2: verifyRes?.data?.message || "Please contact support." });
            }
        } catch (error: any) {
            Toast.show({ type: "error", text1: "Payment error", text2: error?.message || "Something went wrong." });
        } finally {
    ...
    if (!movie) {
        return (
            <View style={{ flex: 1, backgroundColor: colors.background, justifyContent: "center", alignItems: "center", padding: 32 }}>
                <StatusBar barStyle="light-content" backgroundColor="transparent" translucent />
                <View style={{ width: 90, height: 90, borderRadius: 45, backgroundColor: colors.surfaceVariant, justifyContent: "center", alignItems: "center", marginBottom: 24 }}>
                    <Ionicons name={movieDeleted ? "trash-outline" : "film-outline"} size={40} color={colors.textMuted} />
                </View>
                <Text style={{ color: colors.textPrimary, fontSize: 22, fontWeight: "700", marginBottom: 12, textAlign: "center" }}>
                ...
                <TouchableOpacity
                    ...
                    style={{ backgroundColor: colors.accent, borderRadius: 12, paddingHorizontal: 28, paddingVertical: 14, flexDirection: "row", alignItems: "center", gap: 8, width: "100%", justifyContent: "center" }}
                >
        ...
        <View style={{ flex: 1, backgroundColor: colors.background }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                ...
                <View style={{ height: HERO_HEIGHT, position: "relative" }}>
                    ...
                    <TouchableOpacity
                        onPress={() => router.back()}
                        style={{ position: "absolute", top: insets.top + 10, left: 16, width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(0,0,0,0.55)", justifyContent: "center", alignItems: "center" }}
                    >
                        <Ionicons name="arrow-back" size={22} color="#fff" />
                    </TouchableOpacity>
                    {owned && (
                        <View style={{ position: "absolute", top: insets.top + 10, right: 16, backgroundColor: "#1D9E75", borderRadius: 8, paddingHorizontal: 12, paddingVertical: 5 }}>
                            <Text style={{ color: "#fff", fontSize: 12, fontWeight: "700" }}>✓ OWNED · {daysLeft}d left</Text>
                        </View>
                    )}
                </View>
                ...
                <View style={{ paddingHorizontal: 20, marginTop: -8 }}>
                    ...
                    <Text style={{ fontSize: 26, fontWeight: "800", color: colors.textPrimary, marginBottom: 10, lineHeight: 32, letterSpacing: 0.3 }}>
                        {movie.title}
                    </Text>

                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }} contentContainerStyle={{ gap: 8 }}>
                        {categoryLabel && <MetaChip icon="pricetag-outline" label={categoryLabel} colors={colors} />}
                    ...
                    {owned ? (
                        <TouchableOpacity
                            ...
                            style={{ backgroundColor: colors.accent, borderRadius: 12, paddingVertical: 16, marginBottom: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10 }}
                        >
                    ...
                    {/* ── REVIEWS SECTION ── */}
                    <View style={{ borderTopWidth: 0.5, borderTopColor: colors.divider, paddingTop: 20 }}>
                        ...
                        {reviews.length === 0 ? (
                            <View style={{ alignItems: "center", paddingVertical: 32, backgroundColor: colors.surface, borderRadius: 14, borderWidth: 0.5, borderColor: colors.border, marginBottom: 16 }}>
                                <Ionicons name="chatbubble-outline" size={32} color={colors.textMuted} />
                                <Text style={{ color: colors.textMuted, marginTop: 10, fontSize: 14 }}>No reviews yet</Text>
                                {owned && (
                                    <TouchableOpacity onPress={() => setShowReviewModal(true)} style={{ marginTop: 12 }}>
                                        <Text style={{ color: colors.accent, fontSize: 13, fontWeight: "600" }}>Be the first to review</Text>
                                    </TouchableOpacity>
                                )}
                            </View>
                        ) : (
                            <>
                                ...
                                {hasMoreReviews && (
                                    <TouchableOpacity
                                        ...
                                        style={{ alignItems: "center", paddingVertical: 14, backgroundColor: colors.surface, borderRadius: 10, marginTop: 4, borderWidth: 0.5, borderColor: colors.border }}
                                    >
                                        <Text style={{ color: colors.accent, fontSize: 14, fontWeight: "600" }}>Load more reviews</Text>
                                    </TouchableOpacity>
                                )}
                            </>
                        )}
            ...
            {checkingPayment && (
                <View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.82)", justifyContent: "center", alignItems: "center", zIndex: 100 }}>
                    <ActivityIndicator size="large" color={colors.accent} />
                    <Text style={{ color: "#fff", marginTop: 16, fontSize: 15, fontWeight: "600" }}>Verifying payment...</Text>
                </View>
            )}
            ...
            <Modal visible={showReviewModal} transparent animationType="slide" onRequestClose={() => setShowReviewModal(false)}>
                <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
                    <TouchableOpacity style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)" }} activeOpacity={1} onPress={() => setShowReviewModal(false)} />
                    <View style={{ backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingBottom: 32, maxHeight: "82%" }}>
                        <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: colors.border, alignSelf: "center", marginVertical: 12 }} />
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 24, paddingBottom: 16, borderBottomWidth: 0.5, borderBottomColor: colors.divider }}>
                            <Text style={{ fontSize: 18, fontWeight: "700", color: colors.textPrimary }}>{myReview ? "Edit Your Review" : "Write a Review"}</Text>
                            <TouchableOpacity onPress={() => setShowReviewModal(false)} style={{ width: 30, height: 30, borderRadius: 15, backgroundColor: colors.surfaceVariant, justifyContent: "center", alignItems: "center" }}>
                                <Ionicons name="close" size={16} color={colors.textSecondary} />
                            </TouchableOpacity>
                        </View>
                        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 8 }}>
                            <Text style={{ fontSize: 14, color: colors.textSecondary, marginBottom: 10 }}>Your Rating <Text style={{ color: colors.accent }}>*</Text></Text>
                            ...
                            <Text style={{ fontSize: 14, color: colors.textSecondary, marginBottom: 8 }}>Comment (optional)</Text>
                            <TextInput
                                value={reviewComment} onChangeText={setReviewComment}
                                placeholder="Share your thoughts..." placeholderTextColor={colors.inputPlaceholder}
                                multiline numberOfLines={4} maxLength={1000}
                                style={{ backgroundColor: colors.inputBg, borderWidth: 1, borderColor: colors.inputBorder, borderRadius: 12, padding: 14, fontSize: 14, color: colors.inputText, textAlignVertical: "top", minHeight: 100, marginBottom: 8 }}
                            />
                            <Text style={{ fontSize: 11, color: colors.textMuted, marginBottom: 20, textAlign: "right" }}>{reviewComment.length}/1000</Text>
                            <TouchableOpacity
                                onPress={handleSubmitReview} disabled={submittingReview || reviewRating === 0}
                                style={{ backgroundColor: reviewRating === 0 ? colors.surfaceVariant : colors.accent, borderRadius: 12, paddingVertical: 16, alignItems: "center", marginBottom: 8 }}
                            >
                                {submittingReview ? <ActivityIndicator color="#fff" /> : (
                                    <Text style={{ color: reviewRating === 0 ? colors.textMuted : "#fff", fontWeight: "700", fontSize: 16 }}>
...
function MetaChip({ icon, label, colors, iconColor }: { icon: string; label: string; colors: any; iconColor?: string; }) {
    return (
        <View style={{ flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: colors.surfaceVariant, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 0.5, borderColor: colors.border }}>
        ...
        <View style={{ backgroundColor: colors.card, borderRadius: 12, padding: 14, marginBottom: 10, borderWidth: 0.5, borderColor: colors.border }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    <View style={{ width: 34, height: 34, borderRadius: 17, backgroundColor: colors.accent + "25", justifyContent: "center", alignItems: "center" }}>
                        <Text style={{ color: colors.accent, fontSize: 14, fontWeight: "700" }}>{review.userName?.charAt(0)?.toUpperCase() || "U"}</Text>
                    </View>
                    <View>
                        <Text style={{ fontSize: 13, fontWeight: "700", color: colors.textPrimary }}>{review.userName}</Text>
